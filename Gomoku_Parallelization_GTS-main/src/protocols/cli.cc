...
bool RenjuProtocolCLI::beginSession(int argc, char const *argv[]) {
    ...
    if (argc < 2) {
        ...
        std::cerr << "       [-a <algorithm>]  Select Algorithm (1: Negamax, 2: MCTS; default: 1)" << std::endl;
        return false;
    }
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (strncmp(arg, "-s", 2) == 0) {
            ...
            // Algorithm ID
            if (i >= argc - 1) continue;
            parseIntegerArgument(argv[i + 1], 3, &algorithm);
    ...
    std::string result = generateMove(gs_string, ai_player, search_depth, time_limit, num_threads, algorithm);
...
std::string RenjuProtocolCLI::generateMove(const char *gs_string, int ai_player_id, int search_depth,
                                           int time_limit, int num_threads, int algorithm) {
    // Record start time
    double clock_begin = omp_get_wtime();
    ...
    bool success = RenjuAPI::generateMove(gs_string, ai_player_id, search_depth, time_limit, num_threads, algorithm, &actual_depth,
    ...
    double cpu_time = (clock_end - clock_begin);
    //std::clock_t cpu_time = (clock_end - clock_begin);

    double time_req = cpu_time * 1000.0; // ms
    ...
    std::unordered_map<std::string, std::string> data = {{"move_r", std::to_string(move_r)},
                                                         ...
                                                         {"cpu_time", std::to_string(time_req)},
