...
int MyAIEval::evalState(const char *gs, int player) {
    // Check parameters
    if (gs == nullptr ||
...
int MyAIEval::evalMove(const char *gs, int r, int c, int player) {
                        // MyAIEval::MyDirectionPattern* preset_patterns,
                        // int* preset_scores, int preset_patterns_size, int* preset_patterns_skip) {
    
    MyAIEval::MyDirectionPattern *preset_patterns = nullptr;
    ...
    for (bool consecutive = false;; consecutive = true) {
        ...
        int score = MyAIEval::evalADM(adm, preset_patterns_size, 
                                        preset_patterns_skip, preset_patterns, preset_scores);
...
int MyAIEval::evalADM(DirectionMeasurement *all_direction_measurement, int preset_patterns_size,
                        int* preset_patterns_skip, MyAIEval::MyDirectionPattern* preset_patterns,
                        int* preset_scores) {
    int score = 0;
    ...
    for (int i = start_pattern; i < size; ++i) {
        score += MyAIEval::matchPattern(all_direction_measurement, 
                                        &preset_patterns[2 * i]) * preset_scores[i];

        // Only match one threatening pattern
        if (score >= kMyAIEvalThreateningScore) break;
    }

    return score;
}

int MyAIEval::matchPattern(DirectionMeasurement *all_direction_measurement,
...
void MyAIEval::measureAllDirections(const char *gs,
                                       int r,
                                       int c,
                                       int player,
                                       bool consecutive,
    ...
    MyAIEval::measureDirection(gs, r, c, 0,  1, player, consecutive, &adm[0]);
    MyAIEval::measureDirection(gs, r, c, 1,  1, player, consecutive, &adm[1]);
    MyAIEval::measureDirection(gs, r, c, 1,  0, player, consecutive, &adm[2]);
    MyAIEval::measureDirection(gs, r, c, 1, -1, player, consecutive, &adm[3]);
}

void MyAIEval::measureDirection(const char *gs,
                                   int r, int c,
                                   int dr, int dc,
                                   int player,
                                   bool consecutive,
...
void MyAIEval::generatePresetPatterns(MyDirectionPattern **preset_patterns,
                                        int **preset_scores,
    ...
    MyDirectionPattern patterns[_size * 2] = {
    ...
    *preset_patterns = new MyDirectionPattern[_size * 2];
    *preset_scores   = new int[_size];

    memcpy(*preset_patterns, patterns, sizeof(MyDirectionPattern) * _size * 2);
    memcpy(*preset_scores, scores, sizeof(int) * _size);

    *preset_patterns_size = _size;
}

int MyAIEval::winningPlayer(const char *gs) {
