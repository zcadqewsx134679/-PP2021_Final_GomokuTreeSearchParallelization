...
void RenjuAINegamax::heuristicNegamax(const char *gs, int player, int depth, int time_limit, int num_threads, 
                                        bool enable_ab_pruning, int *actual_depth, int *move_r, int *move_c) {
    ...
    if (depth > 0) {
        ...
        // Iterative deepening
        float c_start = omp_get_wtime();
        for (int d = 6;; d += 2) {
            float c_iteration_start = omp_get_wtime();
            ...
            // Times
            float c_iteration = (omp_get_wtime() - c_iteration_start);
            float c_elapsed = (omp_get_wtime() - c_start);
...
int RenjuAINegamax::heuristicNegamax(char *gs, int player, int initial_depth, int depth,
    ...
    if (moves_player.size() == 1 || moves_player[0].heuristic_val >= kMyAIEvalWinningScore) {
    ...
    if (moves_opponent[0].heuristic_val >= kMyAIEvalThreateningScore) {
        block_opponent = true;
        for (int i = 0; i < tmp_size; ++i) {
            auto move = moves_opponent[i];

            // Re-evaluate move as current player
            move.heuristic_val = MyAIEval::evalMove(gs, move.r, move.c, player);
...
void RenjuAINegamax::searchMovesOrdered(const char *gs, int player, std::vector<Move> *result) {
    ...
    //     }
    // }
    std::vector<Move> tmp[g_board_size];
    ...
    for (int r = min_r - 2; r <= max_r + 2; ++r) {
        ...
        for (int c = min_c - 2; c <= max_c + 2; ++c) {
            ...
            m.r = r;
            m.c = c;
            m.heuristic_val = MyAIEval::evalMove(gs, r, c, player);
    ...
    for (int r = min_r - 2; r <= max_r + 2; ++r) {
        for (auto j : tmp[r]) result -> push_back(j);
    }
...
int RenjuAINegamax::negamax(char *gs, int player, int depth, int *move_r, int *move_c) {
    ...
    // Eval game state
    if (depth == 0) return MyAIEval::evalState(gs, player);
